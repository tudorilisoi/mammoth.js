...
function BodyReader(options) {
    ...
    function readFormatting(properties, element) {
        var c = element.first("w:color");
        if (c) {
            var color = c.attributes["w:val"];
            properties.colorValue = color;
            console.log('COLOR ATTR', color);
        }

        var bgc = element.first("w:highlight");
        if (bgc) {
            var bgcolor = bgc.attributes["w:val"];
            properties.bgColorValue = bgcolor;
            console.log('BG COLOR ATTR', bgcolor);
        }

        var f = element.first("w:rFonts");
        if (f) {
            var fontName = f.attributes["w:ascii"];
            properties.fontName = fontName;
            console.log('FONT NAME', fontName);
        }

        var sz = element.first("w:szCs");
        if (sz) {
            var fontSize = sz.attributes["w:val"];
        ...
        var ind = element.first("w:ind");
        //console.log('IND', properties);
        if (ind) {
            var twips = ind.attributes["w:left"];
            if (twips) {
                //inch:cm
                var divider = twips % 720 === 0 ? 720 : 567;
                var guessedLevel = twips / divider;
                if (guessedLevel) {
                    properties.leftIndentLevel = guessedLevel;
                    console.log('LEFT INDENT', twips, properties);
                }
            }
        }
    }
    ...
    function readRunStyle(properties, element) {
        ...
        readFormatting(properties, element);
    }
    ...
    function readBlip(element, blip) {
        ...
        if (embedRelationshipId) {
            imagePath = joinZipPath("word", relationships[embedRelationshipId].target);
            readImage = docxFile.read.bind(docxFile, imagePath);
        } else {
            imagePath = relationships[linkRelationshipid].target;
            readImage = files.read.bind(files, imagePath);
        }
...
function combineResults(results) {
    ...
    return new ReadResult(
        ...
        _.filter(_.flatten(_.pluck(result.value, "extra")), function (x) {
            return x;
        }),
        result.messages
    );
