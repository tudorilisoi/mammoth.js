...
function DocumentConversion(options) {
    ...
    function convertFormatting(res, run, messages) {

        var result = res;

        if (run.colorValue) {
            //result = wrapInNonFreshElement(result, "span", {style: 'color: #' + run.colorValue});
            result = wrapInNonFreshElement(result, "span", {color: '#' + run.colorValue});
        }

        if (run.bgColorValue) {
            result = wrapInNonFreshElement(result, "span", {background: '' + run.bgColorValue});
        }

        if (run.fontName) {
            result = wrapInNonFreshElement(result, "span", {'style': 'font-family:' + run.fontName});
        }

        if (run.fontSize) {
            var normValue = '' + Math.floor(run.fontSize / 2);
            result = wrapInNonFreshElement(result, "span", {'style': 'font-size:' + normValue + 'pt'});
        }

        //handled in convertParagraph
        /*
        if (run.leftIndentLevel) {

            result = wrapInNonFreshElement(result, "li", {});
            result = wrapInNonFreshElement(result, "ul",
                {'class': 'list-indent' + run.leftIndentLevel}
            );
        }*/
    ...
    function convertParagraph(element, messages, callback) {
        ...
        setImmediate(function () {
            convertElements(element.children, messages, function (error, children) {
                ...
                var ret = Html.pathToNode(style, children);
                if (element.leftIndentLevel) {
                    ret = wrapInNonFreshElement(ret, 'li', {});
                    ret = wrapInNonFreshElement(ret, 'ul',
                        {'class': 'list-indent' + element.leftIndentLevel}
                    );
                }
                callback(null, ret);
            });
    ...
    function convertRun(run, messages, callback) {
        ...
        convertElements(run.children, messages, function (error, children) {
            ...
            result = convertFormatting(result, run, messages);
    ...
    function wrapInNonFreshElement(node, tagName, attrs) {
        return Html.nonFreshElement(tagName, attrs || {}, [node]);
    }
